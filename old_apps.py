...
cyclegan = CycleGAN()
cyclegan.load_network(n_epoch_start = 220,path="models/race.h5")
...
#def del_images(translated_path):
#    if os.path.exists(translated_path):
#        os.remove(translated_path)
...
def upload():
    if request.method == 'POST':
        ...
        # Make prediction
        result = cyclegan.test_k_images(im_path = file_path)

        # Process your result for human
        # pred_class = preds.argmax(axis=-1)            # Simple argmax
#        result = "oupuoik.jpg"               # Convert to string
        return result
    return None
